...
from PyQt5.QtWidgets import QTableWidgetItem, QTableWidget, QComboBox, QVBoxLayout, QGridLayout, QDialog, QPushButton, \
    QApplication, QMainWindow, QMessageBox, QLabel, QLineEdit
...
# students(roll INTEGER,name TEXT,gender INTEGER,branch INTEGER,year INTEGER,academic_year INTEGER,address TEXT,
# mobile INTEGER) here gender is either 0 for Male or 1 for Female branch has 6 values ranging from 0 to 5
# 0->Mechanical,1->Civil,2->Electrical,3->ECE,4->CSE,5->IT academic year is when the student joined the college.
...
class DBHelper:
    def __init__(self):
        ...
        self.c.execute(
            "CREATE TABLE IF NOT EXISTS payments(receipt_no INTEGER,roll INTEGER,fee INTEGER,semester INTEGER,receipt_date TEXT)")
    ...
    def searchStudent(self, roll):
        # we make a DB query to search for a student holding the roll number. if we find any then we pass the result
        # returned from the DB to our custom function showStudent() which then analyze the list.
    ...
    # this function is the most important and complex part of the whole program. This adds the payment made by the
    # student to the database. roll and fee are integers and semester it either 0 for Odd semester or 1 for Even
    # semester. there are some possibilities here. They are----- 1.)admin tries to add a fresh entry but he has put
    # semester as Even when the student has not paid the Odd semester fee 2.)admin tries to add entry for a student
    # who has already paid his/her both semester fees 3.)admin tries to add entry for a student for the same semester
    # twice
    ...
    def addPayment(self, roll, fee, semester):
        ...
        try:
            ...
            if not self.c.fetchone():
                ...
                if semester == 1:
                    # query to check if there is any record with same roll number and semester as 0 which is Odd
                    # Semester
                    ...
                    self.c.execute(
                        "INSERT INTO payments (receipt_no,roll,fee,semester,receipt_date) VALUES (?,?,?,?,?)",
                        (receipt_no, roll, fee, semester, date))
                    self.conn.commit()
        ...
        # here in the query we use ORDER BY receipt_no DESC so that rows with semester value as 1 comes first
...
def showStudent(list):
    ...
    # we make the table here. Table has eight rows and 2 columns. in PyQt tables are like grids. you have to place
    # each QTableWidgetItem separately corresponding to the grid system with x and y both starting at 0 index. Then we
    # populate the table with values from the passed list as we got all of them above.
...
def showPaymentFunction(list):
    ...
    # as I said earlier if semester value is 0 that means Odd semester and if it is 1 then student has paid both
    # semester fees as we eliminated the possibility of adding Even semester payment record first.
    if list[3] == 0:
    ...
    table.setItem(4, 1, QTableWidgetItem(str(receipt_date)))
...
# this is the main window which holds everything. It holds for buttons. Enter Student Details Enter Payment Details
# Show Student Details Show Payment Details it has two functions named enterstudent() and enterpayment() which show
# the dialogs created above respectively another two functions showStudent() and showStudentPaymentDialog() shows
# dialog for the user to enter the roll number he or she wants to search records for. showStudent() and showPayment()
# which are the actual functions which call the corresponding searching functions of the DBHelper class. These two
# functions are connected to the 'search' button of the dialog where user enters the roll number.
...
# main function which shows the login dialog first. if user puts the correct username and password it then goes to
# the main window where there are four buttons as mentioned above.
